...
* The `propertyTypes` rule can cause a build failure in cases like `let foo = Foo.bar()` where `Foo` is used as a namespace (e.g. a caseless enum) and `bar()` is a static factory method that returns a different type. For example, `let foo: Foo = .bar()` would be invalid if `bar()` was defined as `static func bar() -> Bar`. As a workaround you can write the type explicitly, like `let foo: Bar = Foo.bar()`, or exclude the type name and/or property name with `--preserve-symbols Foo,bar,etc`.
