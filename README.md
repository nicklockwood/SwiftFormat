...
* The `redundantClosure` rule can sometimes break code if the closure calls a `@discardableResult` function, or one that returns `Never`. To solve this you can either fix it on a per-case basis by adding a `// swiftformat:disable:next redundantClosure` to disable the rule for a specific call site, or you can add `_ =` before the function call to make it explicitly `Void`.
