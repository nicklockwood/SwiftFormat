...
class URLMacroTests: XCTestCase {
    func testBasicURLStringForceUnwrapConverted() {
        ...
        """
        testFormatting(for: input, output, rule: .uRLMacro, exclude: [.blankLineAfterImports, .redundantInit, .propertyTypes, .trailingSpace, .indent, .spaceInsideParens])
    }
    ...
    func testURLStringOptionalNotConverted() {
        ...
        let url = URL(string: "https://example.com")
        """
        testFormatting(for: input, rule: .uRLMacro, exclude: [.blankLineAfterImports, .redundantInit, .propertyTypes, .trailingSpace, .indent, .spaceInsideParens])
    }
